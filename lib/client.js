		...
		function tspLiveDefinition() {
			...
			return {
				...
				buildViewNode: (context) => {
					const state = context.state;
					// 仅在回合进行中、且确实已有模型输出时渲染；turn/end 之后立刻让位给
					// assistant-actions 里的精确 pill（同一数值口径，避免重复显示）。
					if (state === void 0 || state.ended) return null;
					if (state.firstTokenAt === null || !(state.estTokens > 0)) return null;
					return {
						key: context.key,
						kind: TSP_LIVE_KIND,
						id: context.id,
						...
						location: state.location ?? context.start?.location ?? { kind: "unresolved" },
						visibility: "visible",
						data: {
							turn: state.turn,
