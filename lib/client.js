 ...
 *
 * 四个功能各自独立命名空间（locale NS / slot id / data-* 前缀），互不干扰；
...
`.trim();
		...
		*/
		function ModifiedFilesView(props) {
			const { useModifiedFiles, useSession, cwd, openFile, t } = props;
			const h = react.createElement;
...
`.trim();
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				tag.textContent = MSS_CSS + MFS_CSS + WSC_CSS;
				document.head.appendChild(tag);
				return () => tag.remove();
			}, "ui-tools: inject model-select + modified-files + workspace-chip styles");
			...
			const mfsChatSource = (binding) => {
				...
				if (source === void 0) {
					...
					source = {
						getSnapshot: () => target.getSnapshot() ?? runtimeClient.EMPTY_CHAT_SNAPSHOT,
