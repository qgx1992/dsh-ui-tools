...
`.trim();
		...
		function apply(ctx) {
			...
			// 注意：不再搬动 DOM。旧实现挂在 document.body 全局 MutationObserver 上，
			// 用 40ms 定时把工具条节点搬进侧边栏列——框架每次重渲染 slot 都会重建工具条，
			// 搬动又触发观察器，两者互相触发形成无限循环，渲染进程单核 100% 空转、界面卡死。
			// 现在工具条就渲染在 slot 所在位置（侧边栏底部动作区），只做样式注入。
			injectWcStyle();
			ctx.effect(() => () => {
