...
`.trim();
		...
		const WC_CSS = [
			...
			".wc-collapse-bar.dwc-hidden{display:none!important}"
		].join("");
		...
		function apply(ctx) {
			...
			const syncVisibility = (bar) => {
				const width = columnNode === null ? 0 : columnNode.clientWidth;
				bar.classList.toggle("dwc-hidden", width < 64);
				if (resizeObserver === null || resizeTarget !== columnNode) {
					if (resizeObserver !== null) resizeObserver.disconnect();
					resizeTarget = columnNode;
					if (columnNode !== null) {
						resizeObserver = new ResizeObserver(() => {
							const node = document.querySelector(WC_BAR_SELECTOR);
							if (node !== null) node.classList.toggle("dwc-hidden", columnNode.clientWidth < 64);
						});
			...
			const moveBar = () => {
				...
				if (columnNode !== null && parent === columnNode) {
					syncVisibility(bar);
					return;
				}
			...
			}, "ui-tools: workspace-collapse reposition teardown");
